...
class EnsembleCalculator(nn.Module):
    ...
    def forward(self, data: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        data["charge"] = 0.5 * torch.stack(
            [data["charge"] - data["mult"] + 1, data["charge"] + data["mult"] - 1],
            dim=-1,
        )
        outputs = {k: [] for k in self.out_keys}
...
def eval_batched(
    ...
    data,
    batchsize=128,
    in_keys=("coord", "numbers", "charge", "mult"),
    output_device="cpu",
):
    ...
    for idx in tqdm(batch_idx, leave=False):
        o = model({k: v[idx] for k, v in data.items()})
...
def _delta_prp(batch, charges=(0, 1), props=("energy", "charges")):
    ...
    mol_ids, idx0, idx1 = np.intersect1d(
        mol_ids[w_chg0], mol_ids[w_chg1], return_indices=True
    )
    return [
        np.array(batch[p][w_chg0][idx0] - batch[p][w_chg1][idx1]) for p in props
    ], mol_ids
...
def calc_prp(batch, energy_key="energy", charges_key="charges"):
    ...
    (ip, f_el), ids_ip = _delta_prp(
        batch, charges=(1, 0), props=(energy_key, charges_key)
    )
    f_el = -f_el
    ...
    (ea, f_nuc), ids_ea = _delta_prp(
        batch, charges=(0, -1), props=(energy_key, charges_key)
    )
...
def merge_prp(prp_true, prp_pred, keys):
    ...
    ids, idx0, idx1 = np.intersect1d(
        prp_pred["mol_id"], prp_true["mol_id"][w], return_indices=True
    )
    for k in keys:
        d[k] = prp_pred[k][idx0]
...
if __name__ == "__main__":
    ...
    with h5py.File(args.h5file, "r") as f:
        for group_name in tqdm(f):
            ...
            data = {k: v[()] for k, v in g.items()}
            data["charges"] = data["charges"].sum(
                -1
            )  # merge alpha and beta atomic charges

            # make predictions
            # input should be Dict[str, Tensor] containing these keys:
            X = {k: data[k] for k in ("coord", "numbers", "charge", "mult")}
            X = to_tensors(X)
    ...
    print_errors(
        data_ea_ip,
        ("chi", "eta", "omega", "f_rad", "omega_el", "omega_nuc", "omega_rad"),
    )
