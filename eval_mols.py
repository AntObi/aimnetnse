...
def mol2data(mol, device):
    ...
    coord = (
        torch.tensor(coord, dtype=torch.float).unsqueeze(0).repeat(3, 1, 1).to(device)
    )
    numbers = (
        torch.tensor(numbers, dtype=torch.long).unsqueeze(0).repeat(3, 1).to(device)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--in-file",
        nargs="?",
        help="Multi-molecule input file. Extension should be an acceptable to OpnBabel file type.",
    )
    parser.add_argument(
        "--out",
        help="Output multi-line JSON file with computed properties.",
